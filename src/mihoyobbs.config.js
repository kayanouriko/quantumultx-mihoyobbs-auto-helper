...
function main() {
    ...
    const tasks = parse(params, 'tasks', '1,2,3')
    const sections = parse(params, 'sections', '34')
    const actions = parse(params, 'actions', '58,59,60,61')
...
function parse(params, key, placeholder) {
    return (params.get(key) ?? placeholder).split(',').map(e => parseInt(e))
}
